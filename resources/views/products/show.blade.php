    ...
    <div class="row gy-4 mb-5">
        ...
        <div class="col-md-12 mb-5">
            <div class="row justify-content-center">
                <div class="col-md-4">
                    <img src="{{ asset('assets/images/' . $product->primary_image) }}" class="rounded" width=350 height=220 alt="primary-image">
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            ...
            <input disabled type="text" value="{{ number_format($product->discount_price) }}" class="form-control" />
        </div>

        <div class="col-md-3">
            <label class="form-label">تاریخ شروع حراجی</label>
            <input disabled type="text" value="{{ $start_date_discount ?? '' }}" class="form-control" />
        </div>

        <div class="col-md-3">
            <label class="form-label">تاریخ پایان حراجی</label>
            <input disabled type="text" value="{{ $end_date_discount ?? '' }}" class="form-control" />
        </div>
        ...
        <div class="col-md-12">
            @foreach ($product->images as $image)
                <img class="rounded" width="200" src="{{ asset('assets/images/' . $image->image) }}" alt="images">
            @endforeach
        </div>
    </div>
@endsection
